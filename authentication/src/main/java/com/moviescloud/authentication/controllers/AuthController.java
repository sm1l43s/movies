...
import com.moviescloud.authentication.services.PrivilegeClient;
import com.moviescloud.authentication.services.UserClient;
...
public class AuthController {
    ...
    public HttpStatus signup(
        ...
        if (userClient.existsUserByEmail(authentication.getEmail())) {
            throw new UnauthorizedException("Wrong email address or password!");
        }

        List<Privilege> privileges = new ArrayList<>();
        privileges.add(privilegeClient.findByName("GET_REVIEW"));
        privileges.add(privilegeClient.findByName("CREATE_REVIEW"));
        privileges.add(privilegeClient.findByName("GET_STAFF"));
        privileges.add(privilegeClient.findByName("GET_MOVIE"));
        privileges.add(privilegeClient.findByName("VOTES_MOVIE"));

        userClient.save(new User(authentication.getEmail(), passwordEncoder.encode(authentication.getPassword()),
